...
leagueIDs = ['331953']  # <---Input your league IDs
...
for leagueID in leagueIDs:
    ...
    leagueInfo = requests.get('https://fantasy.premierleague.com/api/leagues-classic/331953/standings').json()
    teamList = []
    ...
    for teamID in teamList:
        foo = requests.get('https://fantasy.premierleague.com/api/entry/' + str(teamID) + '/history').json()
        print(foo)
        ...
        dfTemp = pd.DataFrame(foo[u'past'], columns=fields)
    ...
    i = 1
    for key, group in g:
        plt.scatter([i] * len(group), group['Gameweek Points'], s=100, marker='*', label=key,
                    color=plt.cm.Paired(rgbs[i - 1]))
        labels.append(key)
        i = i + 1
    ax.grid(axis='y')
